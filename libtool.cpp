...
void AStarSearcher::search(int *map, int sx, int sy, int ex, int ey){
	...
	CLOSE.reset();
	lastNode = nullptr;
	node *startNode = new node;						//初始节点（起点）
	startNode->parent = nullptr;					//初始节点无前驱
	startNode->tileNum = getTile(sx, sy);			//在节点中存入块号
	startNode->x = sx, startNode->y = sy;			//从块号获取XY坐标并放入节点
	startNode->gPast = 0;							//已经走过的计算出的实际价值
	startNode->hEst = heuristic(sx, sy, ex, ey);	//用启发式函数计算出来的预计价值
	startNode->fValue = startNode->gPast + startNode->hEst;		//总价值
	OPEN.insert(startNode);		//加入OPEN表中
	...
	while (OPEN.getLength() > 0)
	{
		...
		GenerateSucc(map, bestNode, bestNode->x, bestNode->y - 1, ex, ey);			//生成上节点
		//GenerateSucc(map, bestNode, bestNode->x + 1, bestNode->y - 1, ex, ey);		//生成右上节点
		GenerateSucc(map, bestNode, bestNode->x + 1, bestNode->y, ex, ey);			//生成右节点
		//GenerateSucc(map, bestNode, bestNode->x + 1, bestNode->y + 1, ex, ey);		//生成右下节点
		GenerateSucc(map, bestNode, bestNode->x, bestNode->y + 1, ex, ey);			//生成下节点
		//GenerateSucc(map, bestNode, bestNode->x - 1, bestNode->y + 1, ex, ey);		//生成左下节点
		GenerateSucc(map, bestNode, bestNode->x - 1, bestNode->y, ex, ey);			//生成左节点
		//GenerateSucc(map, bestNode, bestNode->x - 1, bestNode->y - 1, ex, ey);		//生成左上节点
	}
	return;
}

void AStarSearcher::GenerateSucc(int *map, node *bestNode, int x, int y, int ex, int ey){
	if (x < 0 || x >= w || y < 0 || y >= h || map[getTile(x, y)] < 0) return;	//这不是通路
	node *oldNode = nullptr;
	int i;
	if ((oldNode = OPEN.find(getTile(x, y))) != nullptr){
		//在OPEN表里找到了这个节点,说明有新的路,那么如果新找到的这条路更短（比较gPast更小，那么新的路更优）
		for (i = 0; i < 4; ++i)
